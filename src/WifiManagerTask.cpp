...
const char *WifiManagerTask::getTaskName()
{
  return "WifiManager";
}

int WifiManagerTask::getTaskCore()
{
  return 0;
}
...
void WifiManagerTask::setup()
{
  ...
  Log.sinfoln("WIFI", PSTR("Start setting WiFI"));
  Log.sinfoln("WIFI", PSTR("WiFi debug level: %u"), (wm_debuglevel_t)WIFI_DEBUG_LEVEL);
  wm.setDebugOutput(settings.debug, (wm_debuglevel_t)WIFI_DEBUG_LEVEL);
  Log.sinfoln("WIFI", PSTR("Set Project Name"));
  wm.setTitle(PROJECT_NAME);
  Log.sinfoln("WIFI", PSTR("Set Menu HTML"));
  ...
  Log.sinfoln("WIFI", PSTR("Add paramters, checkbox, separator ..."));
  ...
  Log.sinfoln("WIFI", PSTR("Set hostname"));
  ...
  Log.sinfoln("WIFI", PSTR("Connect to WiFi if already configured, otherwise go in AP Mode"));
...
void WifiManagerTask::saveParamsCallback()
{
  bool changed = false;
...
void WifiManagerTask::arpGratuitous()
{
